...
export default function Integrations() {
  const [status, setStatus] = useState<IntegrationStatus>({
    configured: false,
    provider: 'resend',
    senderName: 'IVM Optim',
    senderEmail: DEFAULT_SENDER,
    replyTo: DEFAULT_SENDER,
  })
  const [apiKey, setApiKey] = useState('')
  const [showKey, setShowKey] = useState(false)
  const [senderName, setSenderName] = useState('IVM Optim')
  const [senderEmail, setSenderEmail] = useState(DEFAULT_SENDER)
  const [replyTo, setReplyTo] = useState(DEFAULT_SENDER)
  const [recipient, setRecipient] = useState('')
  const [subject, setSubject] = useState('IVM Optim Test Email')
  const [message, setMessage] = useState('This is a successful communication test from IVM Optim.')
  ...
  const loadStatus = async () => {
    ...
    try {
      ...
      if (result.status) {
        ...
        setSenderName(result.status.senderName || 'IVM Optim')
