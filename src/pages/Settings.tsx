...
export default function Settings() {
  ...
  const exportSettings = () => {
    ...
    anchor.href = url
    anchor.download = `ivm-optim-settings-${new Date().toISOString().slice(0,10)}.json`
    anchor.click()
  ...
  const importSettings = async (event: ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      setMessage('The selected file is not a valid IVM Optim settings export.')
    } finally {
      event.target.value = ''
    }
  }
  ...
  return <div className="space-y-5">
    ...
    {tab === 'users' && <Navigate to="/settings/users" replace />}
