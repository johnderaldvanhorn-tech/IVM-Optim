...
export default function AppLayout(){
 ...
 return <div className="min-h-screen bg-slate-50">
  <aside className="fixed inset-y-0 left-0 z-30 w-72 overflow-y-auto bg-slate-950 px-4 py-5 text-white">
   <div className="mb-6 px-2">
    <div className="rounded-2xl bg-white px-4 py-3 shadow-sm ring-1 ring-white/10">
     <img src="/brand/ivm-optim-logo.png" alt="IVM Optim" className="h-auto w-full" />
    </div>
    <div className="mt-3 flex items-center justify-between gap-2">
     <p className="text-xs font-semibold uppercase tracking-[.18em] text-blue-300">{APP_NAME}</p>
     <span className="rounded-md border border-blue-400/30 bg-blue-400/10 px-1.5 py-0.5 text-[10px] font-semibold text-blue-200">{VERSION_LABEL}</span>
    </div>
    <p className="mt-1 text-xs leading-5 text-slate-400">Intelligent vending optimization and decision support</p>
   </div>
   <nav className="space-y-7">{phases.map(phase=><section key={phase.label} className={phase.label.startsWith('Phase 2')?'border-t border-violet-500/30 pt-6':''}><div className={`mb-4 rounded-xl px-3 py-2 text-[11px] font-bold uppercase tracking-[.16em] ${phase.tone==='violet'?'bg-violet-500/15 text-violet-300':'bg-blue-500/10 text-blue-300'}`}>{phase.label}</div><div className="space-y-5">{phase.groups.map(group=><section key={group.label}><div className="mb-1.5 px-3"><p className="text-[10px] font-bold uppercase tracking-[.16em] text-slate-500">{group.label}</p>{group.description?<p className="mt-0.5 text-[10px] text-slate-600">{group.description}</p>:null}</div><div className="space-y-1">{group.items.map(({label,to,icon:Icon,end})=><NavLink key={to} to={to} end={end} className={({isActive})=>`flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm transition-colors ${isActive?(phase.tone==='violet'?'bg-violet-600 text-white shadow-sm shadow-violet-950/40':'bg-blue-600 text-white shadow-sm shadow-blue-950/40'):'text-slate-300 hover:bg-slate-800 hover:text-white'}`}><Icon size={18}/><span>{label}</span></NavLink>)}</div></section>)}</div></section>)}</nav>
  </aside>
  <main className="ml-72 min-h-screen">
   <header className="flex h-20 items-center justify-between border-b border-slate-200 bg-white px-8">
    <div><h2 className="font-semibold text-slate-900">IVM Optim</h2><p className="text-sm text-slate-500">Intelligent vending operations, analytics, and optimization</p></div>
