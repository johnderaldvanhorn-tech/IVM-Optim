...
Deno.serve(async (request) => {
    ...
    return json(
      {
        ok: false,
        stage: 'authorization',
        error: 'Missing Supabase authentication session.',
      },
      401,
    )
  }
  ...
  if (authError || !authData?.user) {
    console.error('[authorization] Invalid Supabase session:', authError)

    return json(
      {
        ok: false,
        stage: 'authorization',
        error: 'Your Supabase session is invalid or expired.',
      },
      401,
    )
  }
  ...
  if (profileError) {
    console.error('[authorization_profile]', profileError)

    return json(
      {
        ok: false,
        stage: 'authorization_profile',
  ...
  const loadStatus = async () => {
    ...
    return {
      ...
      provider: 'resend',
      senderName:
        settings?.sender_name || 'IVM Optim',
      senderEmail:
          ...
          body: JSON.stringify({
            ...
            html:
              ...
              `<p>${message
                ...
                .replace(/\n/g, '<br>')}</p>` +
              `<hr>` +
              `<p style="font-size:12px;color:#64748b">Sent by the IVM Optim.</p>` +
              `</div>`,
          }),
        },
      )
